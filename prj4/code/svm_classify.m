...
for i = 1 : num_categories
    matching_index = strcmp(categories{i}, train_labels);
    labels = double(-1*ones(1, N));
    labels(matching_index) = labels(matching_index) + 2;
    [w,b] = vl_svmtrain(train_image_feats', labels, 0.00006);
    all_w(:, i) = w;
    all_b(1, i) = b;
end

confidences = test_image_feats * all_w + all_b(ones(N, 1),:);
